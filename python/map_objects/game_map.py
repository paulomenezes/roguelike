...
from item_functions import cast_confuse, cast_fireball, cast_lighning, heal
    ...
    for i in range(number_of_items):
      ...
      if not any([entity for entity in entities if entity.x == x and entity.y == y]):
        ...
        if item_chance < 25: # 70, 85, ...
          ...
          item_component = Item(use_function=cast_confuse, targeting=True, targeting_message=Message('Left-click a target tile for the fireball, or right-click to cancel.', libtcod.light_cyan))
          item = Entity(x, y, '#', libtcod.light_pink, 'Confusion Scroll', render_order=RenderOrder.ITEM, item=item_component)
        else:
          item_component = Item(use_function=cast_lighning, damage=20, maximum_range=5)
