	...
	while not libtcod.console_is_window_closed():
		...
		if move and game_state == GameStates.PLAYERS_TURN:
			...
			for entity in entities:
				...
				message_log.add_messages(Message('There is nothing here to pick up', libtcod.yellow))
		...
		if inventory_index is not None and previous_game_state != GameStates.PLAYER_DEAD and inventory_index < len(player.inventory.items):
			...
			if game_state == GameStates.SHOW_INVENTORY:
				print('use 1')
				abc = player.inventory.use(item, entities=entities, fov_map=fov_map)
				print(abc)
				player_turn_results.extend(abc)
		...
		for player_turn_result in player_turn_results:
			...
			if message:
				message_log.add_messages(message)
			...
			if targeting:
				...
				message_log.add_messages(targeting_item.item.targeting_message)

			if targeting_cancelled:
				game_state = previous_game_state
				message_log.add_messages(Message('Targeting cancelled'))
